...
def create_table():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS medicines
                 (id INTEGER PRIMARY KEY, name TEXT, expiry_date TEXT, installation_date TEXT, quantity INTEGER, days_remaining INTEGER)''')
    conn.commit()
    conn.close()
...
def update_days_remaining():
    ...
    for med in medicines:
        ...
        c.execute("UPDATE medicines SET days_remaining=? WHERE id=?", (days_until_expiry, med[0]))
        conn.commit()
...
# Schedule job to update days remaining every day at midnight
schedule.every().day.at("00:00").do(update_days_remaining)
